...
#         self.assertEqual(data['message'], 'resource not found')
    ...
    def test_add_movie(self):
        ...
        resp = requests.post(url + '/movies', data='{"title": "Heroku Geography","release_date":"2010/02/16"}', headers=auth_header)
        print("test_add_movie: Got response:{} {}".format(resp, type(resp)))
        data = resp.json()
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(data['success'], True)
