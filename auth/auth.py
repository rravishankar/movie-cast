...
def verify_decode_jwt(token):
    # print("Entered verify_decode_jwt with token:", token)
    jsonurl = urlopen(f'https://{AUTH0_DOMAIN}/.well-known/jwks.json')
    # print("verify_decode_jwt:L1")
    jwks = json.loads(jsonurl.read())
    # print("verify_decode_jwt:L2")

    unverified_header = jwt.get_unverified_header(token)
    # print("GOt unverified header", unverified_header)
    rsa_key = {}
    ...
    if rsa_key:
        try:
            ...
            raise AuthError({
                'code': 'invalid_claims',
                'error': 'Incorrect claims. Please, check the audience\
                     and issuer.'
            }, 401)
        except Exception:
            raise AuthError({
...
@TODO implement @requires_auth(permission) decorator method
    ...
    check the requested permission return the decorator
    ...
    def requires_auth_decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                abort(401)
            # print("Now calling check_permission")
            check_permissions(permission, payload)
            # return f(payload, *args, **kwargs)
            if(kwargs.get('id')):
